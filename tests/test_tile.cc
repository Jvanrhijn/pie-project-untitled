...
TEST(TestTile, AddReachable){
  ...
  auto new_tile = std::make_shared<pie::Tile>(3, 4); //make arbitrary new tile

  tile.add_reachable(new_tile);
  ASSERT_EQ(1, tile.reachables().size());

  auto reachables = tile.reachables();
  ASSERT_EQ(reachables[0]->coordinates(), std::make_pair(3ul, 4ul)); //take the first element of the vector reachables, which is a tile, and dereference the pointer to that tile and output it's coordinates.
}
