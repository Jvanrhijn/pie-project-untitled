...
void Character::Draw(GLFWwindow *window) const {
  ...
  mat4x4_scale_aniso(model, model, scale_, scale_, 1.0f);
...
void Character::BuildVertices() {
  ...
  for (size_t i=0; i<6; i++) {
    vertex_data_[i*stride + 0] = xpos + size_.first*(i == 0 || i == 1 || i == 3? 0.0f : 1.0f);
    vertex_data_[i*stride + 1] = ypos + size_.second*(i == 1 || i == 2 || i == 4? 0.0f : 1.0f);
  }
